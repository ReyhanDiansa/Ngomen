...
import { AddMusicReview } from "@/utils/MusicAPI";

const AddReviewMus = ({ isOpen, onClose, musicId }) => {
  ...
  const handleSendMusic = async () => {
    ...
    if (isValid) {
      try {
        await AddMusicReview({ reviewData: data });
      ...
      <div className="overflow-auto bg-white p-6 rounded-md w-full md:mx-0 mx-4 md:w-[30rem] max-h-[35rem] md:max-h-[30rem]">
        ...
        <div>
          ...
          <p
            ...
            className="mt-2 text-xs text-gray-500 dark:text-gray-400"
          >
            Enter your comment, review, and critiques for this music
          </p>
        </div>
...
export default withAuth(AddReviewMus);
